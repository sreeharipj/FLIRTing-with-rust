...
`low_pc`/`high_pc` are skipped, and tombstoned DIEs — address 0, debug info for code the linker
discarded, 48.4% of instances in a `cgu=16` non-LTO build and 0.05% under fat LTO — are excluded
as dead.
...
Softening the alphabet — dropping register identity, or going mnemonic-only — buys 5–7 points at
identity level, so the alphabet stays as it is. Reproducibility falls as fragments get longer,
...
about 5×10⁴ hashes per binary, and what survives lands on a true inline boundary 91–97.5% of the
time. The identity yield also beats the whole-function database on the same corpus: a median 683
...
| inline starts that are also a CFI row | 202 — 0.8% recall |
| interior CFI rows that are an inline start | 130 — 1.7% precision |
...
`lto-fat` and `optz`, toolchain 1.97.1. Sanity check first — `base→base` third-party function
...
| either — a coarser label is still exact | **84.8%** | 48.2% |
...
**Emit the common denominator of an ambiguous hash.** 77.1% of matched-flag misses are
byte-identical bodies discarded by `ambiguity ⇒ drop`, and 84.8% of ambiguous whole-function
hashes have all their colliding identities sharing a crate or a method name. Emitting that
denominator — `alloc::*`, `*::fmt` — as its own output class is strictly additive, since those
functions are UNKNOWN today, and it moves them into LIBRARY for triage. It needs its own metric:
...
**Build the fragment database.** Every internal test passes and all of them are within-corpus.
The cross-corpus run against held-out targets is the gate, and it should be predeclared — a bar
for fragment naming precision, decided in advance, that decides whether the subsystem ships.
...
- The "same inline multiset, bytes differ" bucket is n=42. Its profile — median token similarity
  0.97, longest single common run only 0.76 of the shorter body, 4.8% of pairs with one run ≥0.95
  — indicates many small scattered edits rather than one splice. That rules out content-defined
